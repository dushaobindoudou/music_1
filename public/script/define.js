...
wky_define("wky", function(wky){
    ...
    var bro = {};
    var tem = "";
    var broInfo = ua.match(/(opera|chrome|safari|firefox|msie)\/?\s*(\.?\d+(\.\d+)*)/i);
    if (broInfo) {
        if (broInfo[1]) {
            bro["name"] = broInfo[1].toLowerCase();
        }
        bro["version"] = broInfo[2] || 1;
    }
    return {
        varName: "browser",
        varVal: bro
...
wky_define("wky.core", function(core){
    ...
    var toArray = function(nodes){
        ...
        try {
            if (ta) {
                return ta.call(nodes);
            }
        } 
        catch (e) {
            var i = nodes.length, results = [];
            while (i--) {
                results[i] = nodes[i];
            }
            return results;
    ...
    //json to paramer
    var jsonToParam = function(obj){
        if (isUndefined(obj)) {
            return "";
        }
        var param = [];
        forIn(obj, function(k, v){
...
wky_define("wky.tween", function(){
    ...
    var animate = function(option){
        ...
        //校准,
        if (duration / fps !== parseInt(duration / fps)) {
            duration = parseInt(duration / fps) * fps;
        }
        
        var anFn = function(){
            var delta = tween[easing](init * fps / duration);
            var clVal = start + (delta * change);
            //console.log(delta + ":" + clVal);
            step(delta, clVal, change);
            if (init >= duration / fps) {
...
wky_define("wky.dom", function(){
    ...
    var setIeFilter = function(ele, val){
        var ralpha = /alpha\([^)]*\)/i;
        var ropacity = /opacity\s*=\s*([^)]*)/;
        
        var style = ele.style;
        var currentStyle = ele.currentStyle;
        var opacity = core.isNumber(val) ? "alpha(opacity=" + val * 100 + ")" : "";
        var filter = currentStyle && currentStyle.filter || style.filter || "";
        //ie 没有hasLayerout 会有问题
        style.zoom = 1;
        //移除filter
        if ((val >= 1 || val === "") && str.trim(filter.replace(ralpha, "")) === "" && style.removeAttribute) {
            style.removeAttribute("filter");
            if (val === "" || currentStyle && !currentStyle.filter) {
                return;
            }
        }
    ...
    var setStyleToEle = function(ele, name, val){
        var broName = browser.name;
        var version = browser.version;
        var styName = str.toCamelCase(name);
        if (styName === "float" || styName === "cssFloat") {
            if (broName === "msie") {
                styName = "styleFloat";
            }
            else {
        ...
        if (styName != "opacity") {
            ele.style[styName] = val;
            return;
        }
        if (broName !== "msie" && version > 9) {
    ...
    var setStyle = function(ele, name, val){
        ...
        if (core.isString(name)) {
            ...
            core.forIn(name, function(k, v){
                ...
                setStyleToEle(ele, k, v);
            })
        }
    }
    
    
    var currentCss = function(){
        var getStyles;
        var curCSS;
        ...
        if (window.getComputedStyle) {
            ...
            curCSS = function(elem, name, _computed){
                ...
                // getPropertyValue ie9下 得到filter
    ...
    var getStyle = function(){
        return currentCss.apply(null, arguments);
    }
    ...
    var getUnits = function(target, prop){
        ...
        var factors = {}; // 比率
        var units = ""; // 转换后的值
        var value = getStyle(target, prop); // 属性值
        var numeric = value.match(/\d+/); // 数字
        if (numeric === null) { // 返回空
            throw "Invalid property value returned";
        }
        numeric = numeric[0]; //匹配的数字值
        var unit = value.match(/\D+$/); //单位符号
        unit = (unit == null) ? map.pixel : unit[0]; // 没有单位直接设置为px
        var activeMap; // 符号转换成对应的字母
        for (item in map) {
            if (map[item] == unit) {
                activeMap = item;
                break;
        ...
        if (!activeMap) { // 如果没找到 说明没有这个单位
            throw "Unit not found in map";
        }
        var parentEle = target.parentElement;
        var offWidth = parentEle.offsetWidth;
        units = numeric / baseline * offWidth + "px";
        return units; // returns the object with converted unit values...
    }
    
    //淡入
    var fadeIn = function(options){
        var ele = options.ele;
        var complete = options.complete ||
        function(){
        };
        if (!core.isElement(ele)) {
            return;
        }
        ...
        tween.animate({
            ...
            step: function(delta, clVal){
                dom.setStyle(ele, "opacity", clVal);
            },
            complete: function(){
...
wky_define("wky.dom", function(dom){
    ...
    //append
    var append = function(parent, ele){
        if (!core.isNode(parent) || parent.nodeType !== 1 && parent.nodeType !== 11) {
            return;
        }
        var fragment = document.createDocumentFragment();
        if (core.isString(ele)) {
            parent.insertAdjacentHTML("beforeend", ele);
            return;
        }
        else {
    ...
    //prepend
    var prepend = function(parent, ele){
        insert(parent, ele, 0);
    }
    
    //insert
    var insert = function(parent, ele, pos){
        if (!core.isNode(parent) || parent.nodeType !== 1 && parent.nodeType !== 11) {
            return;
        }
        var fragment = document.createDocumentFragment();
        ...
        pos = parseInt(pos);
        pos = isFinite(pos) ? pos : 0;
        if (pos < 0) {
            pos = len + pos;
        }
        var childEle = childs[pos];
        if (core.isString(ele)) {
            if (childEle && childEle.insertAdjacentHTML) {
                childEle.insertAdjacentHTML("beforebegin", ele);
            }
            return;
        }
        else {
        ...
        parent.insertBefore(fragment, childEle);
    }
...
wky_define("wky", function(){
    ...
    var domReady = function(callback){
        ...
        var ready = function(){
            isDomReady = true;
            core.each(callList, function(i, v){
...
wky_define("wky.event", function(){
    ...
    //todo:这个实现性能有点差,回头想一个比较好的方法
    var getAllMatchsEle = function(selector, context){
        if (!core.isString(selector)) {
            return;
        }
        var eles = dom.search(selector, context);
        return eles;
    }
    
    var matchEle = function(node, allEle){
        var eles = allEle || [];
        var clsEle = null;
        core.each(eles, function(i, k){
            if (node === k) {
                clsEle = k;
                return false;
            }
    ...
    var closest = function(node, selector, context){
        var matchedEle = null;
        var allEle = getAllMatchsEle(selector, context);
        
        while (node && !matchedEle) {
            matchedEle = matchEle(node, allEle);
            if (node === context) {
                matchedEle = context;
                break;
            }
    ...
    //委托一个事件
    var delegate = function(node, selector, evt, fn){
        add(node, evt, function(evt){
            var tar = evt.target || evt.srcElement;
            var closeEle = closest(tar, selector, node);
            evt.delegateElement = closeEle;
